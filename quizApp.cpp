...
public:
	...
	void load(string ques, string ans, vector<TF>& questions) // store the question
	{
		...
		for (unsigned int i = 0; i < questions.size(); i++) if (questions[i].sentence == ques) { // check for duplicates
			found = 1;
			break;
		}
...
public:
	...
	void load(string ques, string ans, vector<COMPLETE>& questions) // store the question
	{
		int found = 0;
		SetFormat(ques); // Set the format for the question
		for (unsigned int i = 0; i < questions.size(); i++) if (questions[i].sentence == ques) { // check for duplicate question
			found = 1;
			break;
		}
...
public:
	...
	string q_mcq(int& score) {
		...
		int userChoice_index = (int)(input[0] - 97); // get the index of the letter entered by the user {a:0, b:1, c:2, d:3}
		if (input[0] < 97)
...
public:
	...
	void start_quiz(vector<TF> tf_v, vector<COMPLETE> complete_v, vector<MCQ> mcq_v) { // pass vectors storing the questions to the function
		system(clear);
		if (tf_v.size() < numofTF || complete_v.size() < numofComplete || mcq_v.size() < numofMCQ) cout << "Not enough questions to start quiz" << endl;
		else {
...
public:
	...
	void score_stats(int& exit) { // print score statistics for all quizzes combined
		...
		if (numofQuizzes == 0) cout << "No statistics found to show yet" << endl; // if no quizzes were taken yet
			...
			int maxScore = quiz[0].mcq_stored.size() * 2 + quiz[0].complete_stored.size() * 3 + quiz[0].tf_stored.size(); // maximum score to be had per quiz
	...
	void all_scores() { // print score statistics for each quiz
		...
		for (unsigned int i = 0; i < quiz.size(); i++) {
			...
			cout << "	Number of Complete wrong answers: " << quiz[i].complete_stored.size() - quiz[i].complete_s / 3 << endl;
...
public:
	...
	start:
		...
		cout << "**********************************" << endl;
		cout << setw(x) << "First Name" << setw(x) << "Last Name " << setw(x) << "Username" << setw(x) << "Role" << endl; // print column titles
		for (unsigned int i = 0; i < users.size(); i++) {
	...
	void load_file(vector<TF>& tf_v, vector<COMPLETE>& complete_v, vector<MCQ>& mcq_v) {
		system(clear);
	...
	void view_questions(vector<TF>& tf_v, vector<COMPLETE>& complete_v, vector<MCQ>& mcq_v) {
	retry:
	...
	void add_question(vector<TF>& tf_v, vector<COMPLETE>& complete_v, vector<MCQ>& mcq_v) {
		system(clear);
	...
	retry:
		...
		else if (choice == '3') // add an mcq question
		{
			...
			if (add_answer.length() == 0 || add_answers[0].length() == 0 || add_answers[1].length() == 0 || add_answers[2].length() == 0) {
		...
		bool unchanged = (originalComplete == complete_v.size()) && (originalMCQ == mcq_v.size()) && (originalTF == tf_v.size()); // if the question vector sizes remain unchanged
		if (unchanged == false) cout << "Question added successfully" << endl; // question was added successfully
...
retry:
	...
	bool cond = a->first.length() == 0 || a->last.length() == 0 || a->name.length() == 0 || a->password.length() == 0; // check for any empty inputs
	if (cond) {
		system(clear);
...
retry:
	int id = Login(users); // index of the current user in the vector (-1 if user isn't found, -2 if entered a wrong password)
	if (id != -1 && id != -2) {
		currentUser = users[id];
		string acc_type = (currentUser->getType()) ? "ADMIN" : "PLAYER"; // check if user is admin or player using getter
	menu:
		cout << "Welcome " << currentUser->first << " (" << acc_type << "), please choose from the following options:" << endl;
		...
		cin >> choice;
		while (!(choice >= '1' && choice <= (char)(currentUser->menu_options + 48))) { // menu_options : number of items in the menu for the current user type
			cin.ignore(9999, '\n');
			cout << "Invalid choice, please try again...\n";
			cin >> choice;
		}
		...
		if (choice != (char)(currentUser->menu_options + 48) && exit == 0) goto menu; // as long as the user doesn't choose the last option, view the menu again
		system(clear);
...
void SetFormat(string& variable)	// Set the first letter and each letter after a space to uppercase (string's varible)
{
